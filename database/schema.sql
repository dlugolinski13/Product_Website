...
-- Idempotent: each table is only created if it doesn't already exist, so this file can be
-- re-run safely (e.g. against a fresh environment) without erroring on existing tables.
...
IF OBJECT_ID('dbo.product_classes', 'U') IS NULL
BEGIN
...
IF OBJECT_ID('dbo.terms_and_conditions', 'U') IS NULL
BEGIN
