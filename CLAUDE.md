...
- **API module system**: `/api` is CommonJS (`require`/`module.exports`) — Azure Functions on Static Web Apps failed to load any routes the one time this was switched to ESM (`"type": "module"` deployed to production and every `/api/*` route 404'd), so it must stay CJS. Do not add `"type": "module"` to `api/package.json`.
- **Mocking CJS dependencies in tests**: `vi.mock()` does not reliably intercept `require()` calls made from inside a CommonJS module (it only reliably catches the test file's own direct imports), so API tests use `api/src/testUtils/mockRequire.js` instead — it swaps entries in Node's real `require.cache` for the target module's dependencies before requiring it via `createRequire(import.meta.url)`, then restores the cache. See any `api/src/**/*.test.js` for the pattern (`loadWithMocks(nodeRequire, { '<specifier>': <fakeExports> }, './target.js')`).
- **Testing Azure Functions handlers**: `@azure/functions` is mocked (via the same `loadWithMocks` helper) so `app.http(name, { handler })` just records the handler in a `Map` instead of registering with the real runtime; tests pull the handler out of that map and invoke it directly with a fake `request`/`context`. See `api/src/functions/*.test.js`.
